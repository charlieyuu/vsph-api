...
使用阿里云的推送服务，移动端接入SDK，后端通过调用阿里云的接口向移动端推送问卷填写消息，用户点击后跳转到应用内问卷填写页面。
...
```
- 响应说明：
  - `data`中为一次问卷的信息，示例中给出了单选题、评分题和填空题的json格式，包括题目嵌套关系，不同类型的题目形式可参考形式可参考http://8.152.200.205/questionnaire/
